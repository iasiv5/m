...
| 插件 | 包含内容 |
| --- | --- |
| `iasi` | `Ask Pro Max` agent、`Plan Pro Max` agent、9 个 skills（4 个本地适配：`brainstorming`/`writing-plans`/`handoff`/`cleanup`；5 个上游同步：`grilling`/`codebase-design`/`domain-modeling`/`grill-with-docs`/`improve-codebase-architecture`）、`goal-driven` 命令、`copilot-thought-logging` instruction，以及 hooks 占位资源 |
...
- 输入 `/`，确认可以看到 9 个 skills（`brainstorming`、`writing-plans`、`handoff`、`cleanup`、`grilling`、`codebase-design`、`domain-modeling`、`grill-with-docs`、`improve-codebase-architecture`）以及 `goal-driven` 命令
- 发送一条需求澄清、实施计划、交接总结、最终沉淀或只读分析请求，或手动切换到 `Ask Pro Max` / `Plan Pro Max`，确认相关插件资产可正常调用
