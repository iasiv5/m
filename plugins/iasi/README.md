...
`iasi` 是当前 marketplace 里的 GitHub Copilot 插件，聚焦高标准只读分析、结构化规划与高保真交接。
...
`ask-pro-max` 用于高标准、只读的问题分析；`brainstorming` 用于先澄清需求并落设计文档；`writing-plans` 用于把设计文档转成可执行计划；`handoff` 用于为新 chat 生成基于证据的交接上下文。
...
- 或在你发起需求澄清、实施计划或交接总结等请求时，让 Copilot 根据 skill 描述自动加载

## 当前边界

- 已包含：1 个 custom agent、3 个 skill、1 个 command prompt、1 条 workspace-wide instruction、1 个空 hooks 占位文件
- 未包含：非空 hooks 实现
- 未包含：MCP server
- 未包含：依赖本地绝对安装路径的分发逻辑
