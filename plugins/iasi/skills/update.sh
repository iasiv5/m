#!/usr/bin/env bash
# update.sh — 更新本目录（m/plugins/iasi/skills）下从 external 上游同步的 skills
# 用法:
#   ./update.sh                      # 更新 .your-skill-collection.json 中 skills 字段的所有 skills
#   ./update.sh codebase-design      # 只更新指定的 skill
# 注意: skills 字段从 external 上游拉取；local 字段登记本地适配 skill（brainstorming/writing-plans/cleanup/handoff），手动维护、update.sh 不碰
...
local_skills = set(data.get('local', []))
...
while IFS=$'\t\r' read -r name repo mode payload; do
  printf '  %s\n' "$name"
  ...
  if [[ "$mode" == "files" ]]; then
    local_err=0
    while IFS=$'\t\r' read -r src_path dest_name; do
