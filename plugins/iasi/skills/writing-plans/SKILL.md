...
如果还没有批准过的设计或达成共识，选一条设计轨道做扎实：`brainstorming`（比较方案 / 多选澄清）或 `grill-with-docs`（对抗式压力测试已有想法）；只是缺几条约束时，直接补一份更清晰的需求即可。

## 红灯与反例

下面是本 skill 已知会翻车的几种情况（按 test-prompts 经验面挑选）。命中任一信号，按对应动作处理，不要继续硬写计划：

- **设计未批准 / 方案未定就硬写计划**（`unapproved-design-stop`）→ 信号：用户还在比较方案或没给确认 → 动作：走下方"设计未就绪"三选 fork，不硬凑。
- **编造文件、命令或测试框架**（`missing-design-path-stop`）→ 信号：找不到真实路径就开始编 → 动作：STOP 说明缺口，不猜路径、不猜命令。
- **一个请求横跨多个独立子系统**（`multi-subsystem-split`）→ 信号：账号 / 计费 / 通知混在一起 → 动作：STOP 先拆成多份计划，每份独立可验证。
- **跳过文件结构直接列任务**（`skip-file-structure-first`）→ 信号：先写任务清单再想文件落点 → 动作：回 PHASE 2 先规划文件结构与职责。

其余如"不要占位符""不要省略接口契约""交接终点是 handoff"等已在硬约束与各 PHASE 里正面规定，此处不重复。
...
- 🛑 STOP：设计未批准、需求仍在比较、关键约束缺失或逻辑仍有冲突。动作：不硬凑计划，选一条设计轨道把设计做扎实——
  - `brainstorming`：还在比较方案、需要多选澄清、目标 / 边界没钉住时
  - `grill-with-docs`：已有想法或草稿、想用对抗式追问压力测试它（顺带沉淀 ADR / glossary）时
  - 直接补一份更清晰的需求：只是缺几条约束、不必走完整设计流程时
...
- 选一条设计轨道补设计：`brainstorming`（比较方案 / 多选澄清）或 `grill-with-docs`（对抗式压力测试）；只是缺约束时直接补更清晰的需求
