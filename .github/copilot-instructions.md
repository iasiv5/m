...
When adding or modifying a skill under `plugins/*/skills/`, keep the parent folder name identical to the `name` field in `SKILL.md`, preserve referenced resource paths, and avoid introducing environment-specific setup unless it is clearly documented.
...
- plugin-level metadata belongs in `plugins/<name>/.github/plugin/plugin.json`
- runtime assets belong inside the smallest affected plugin subtree (`agents/`, `commands/`, `skills/`, `instructions/`, or `hooks/`)
